	...
	if(!empty($_REQUEST['filename'])) {
		...
		if( is_wp_error( $tmp ) ) {
			...
			if($result === 10) {
				...
				echo json_encode(array('status' => 10, 'status_text' => 'Timeout detected.', 'message' => 'Timeout detected. Ajax restart.', 'texts' => $message));
				die(-1);
			}
		}
		@unlink( $tmp );
						...
						if(isset($_REQUEST['ids']) && !empty($_REQUEST['ids'])) {
								...
								if( !file_exists($file_content) ) {
									echo 'Error: demo-content file doesn exists.';
										...
										if($result === 10) {
											$message = ob_get_clean();
											echo json_encode(array('status' => 10, 'status_text' => 'Timeout detected.', 'message' => 'Timeout detected. Ajax restart.'));
											die(-1);
										}
								}
								@unlink( $file_content );
